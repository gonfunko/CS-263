...
#  endif
    ...
    if (strcmp(png_libpng_ver, PNG_LIBPNG_VER_STRING))
    {
        ...
        sprintf(str, "  png.h version: %s\n", PNG_LIBPNG_VER_STRING);
        sprintf(str, "  png.c version: %s\n\n", png_libpng_ver);
    }
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        else if (!strncmp(argv[i], "-iccp", 5))
        {
            ...
            if ((iccp_fn = FOPEN(iccp_file, "rb")) == NULL) {
                sprintf(str, "Could not find file: %s\n", iccp_file);
                iccp_length = 0;
            }
            else
            {
...
#  endif
        ...
        {
            sprintf(str, " pngcrush ");
            sprintf(str, PNGCRUSH_VERSION);
            sprintf(str, ", uses libpng ");
            sprintf(str, PNG_LIBPNG_VER_STRING);
            sprintf(str, " and zlib ");
            sprintf(str, ZLIB_VERSION);
            sprintf(str, "\n Check http://pmt.sf.net/\n");
            sprintf(str, " for the most recent version.\n");
            verbose = 0;
        }
    ...
    /* Use of compression window size 256 is not recommended. */
        ...
        sprintf(str, "Invalid window size (%d); using window size=4\n",
...
#  endif
                  mng_ptr = png_create_write_struct(PNG_LIBPNG_VER_STRING,
                    ...
                    (png_error_ptr) NULL);
               if (mng_ptr == NULL)
                  sprintf(str, "pngcrush could not create mng_ptr");

               if ((mng_out = FOPEN(mngname, "wb")) == NULL)
               {
                  sprintf(str, "Could not open output file %s\n",
                          mngname);
                  FCLOSE(fpin);
                  exit(1);
               }
        ...
        if (already_crushed)
        {
            sprintf(str, "   File %s has already been crushed.\n", inname);
        }
        ...
        for (trial = 0; trial <= MAX_METHODS; trial++)
        {
            ...
            if (trial == MAX_METHODS)
            {
                     ...
                     things_have_changed == 0 && nosave == 0))
                {
                    ...
                    if ((fpin = FOPEN(inname, "rb")) == NULL)
                    {
                        sprintf(str, "Could not find input file %s\n",
                                inname);
                        continue;
                    }
    ...
    !defined(PNG_sTER_SUPPORTED) || !defined(PNG_tIME_SUPPORTED)
                 ...
                 */
                {
                         ...
                         &compression_method, &filter_method))
                    {
                        ...
                        if (verbose > 1 && last_trial)
                        {
                            sprintf(str, "   IHDR chunk data:\n");
                            sprintf(str,
                                    "      Width=%lu, height=%lu\n",
                                    (unsigned long)width,
                                    (unsigned long)height);
                            sprintf(str, "      Bit depth =%d\n",
                                    bit_depth);
                            sprintf(str, "      Color type=%d\n",
                                    color_type);
                            if (output_color_type != color_type)
                                sprintf(str,
                                        "      Output color type=%d\n",
                                        output_color_type);
                            sprintf(str, "      Interlace =%d\n",
                                    interlace_method);
                        }
                             ...
                             output_color_type == 4))
                        {
                            if (verbose > 0 && last_trial)
                            {
                                    sprintf(str, "   Reducing truecolor "
                                      "image to grayscale.\n");
                            }
#ifdef PNG_FIXED_POINT_SUPPORTED
...
#if defined(PNG_READ_gAMA_SUPPORTED) && defined(PNG_WRITE_gAMA_SUPPORTED)
                {
                    if (force_specified_gamma)
                    {
                        if (last_trial)
                        {
                            things_have_changed = 1;
                            if (verbose > 0)
                                sprintf(str, "   Inserting gAMA chunk with "
#ifdef PNG_FIXED_POINT_SUPPORTED
                                  "gamma=(%d/100000)\n",
#else
                                  "gamma=%f\n",
...
#  endif
                        ...
                        else if (file_gamma != 0)
                        {
                            if (verbose > 0 && last_trial)
                            {
                                sprintf(str, "   Ignoring sRGB request; "
#  ifdef PNG_FIXED_POINT_SUPPORTED
                                  "gamma=(%lu/100000)"
                                  " is not approx. 0.455\n",
...
#  endif
...
#if defined(PNG_READ_pHYs_SUPPORTED) && defined(PNG_WRITE_pHYs_SUPPORTED)
                {
                    ...
                    if (resolution == 0)
                    {
                        ...
                        png_set_pHYs(write_ptr, write_info_ptr, res_x,
                                     res_y, unit_type);
                        if (verbose > 0 && last_trial)
                            sprintf(str, "   Added pHYs %lu %lu 1 chunk\n",
                            (unsigned long)res_x,
                            (unsigned long)res_y);
                    }
                }
...
#if defined(PNG_READ_tRNS_SUPPORTED) && defined(PNG_WRITE_tRNS_SUPPORTED)
                {
                    ...
                    else if (have_trns == 1)
                    {
                        ...
                        if (verbose > 1)
                            sprintf(str, "Have_tRNS, num_trans=%d\n",
                                    num_trans);
                        if (output_color_type == 3)
                        {
                            trans_values = NULL;
...
#  endif
 ...
 || (defined(PNG_READ_zTXt_SUPPORTED) && defined(PNG_WRITE_zTXt_SUPPORTED))
       ...
       /* (PNG_READ_zTXt_SUPPORTED and PNG_WRITE_zTXt_SUPPORTED) */
            ...
            Catch(msg) {
                if (nosave == 0)
                    sprintf(str, "While converting %s to %s:\n", inname,
                      outname);
                else
                    sprintf(str, "While reading %s:\n", inname);
                sprintf(str,
                ...
                png_destroy_read_struct(&read_ptr, &read_info_ptr,
                                        &end_info_ptr);
                if (verbose > 1)
                    sprintf(str, "returning after cleanup\n");
                trial = MAX_METHODS + 1;
            }
	      ...
	      //              utime(outname, &utim);
            ...
            if (verbose > 0)
            {
                ...
                if (idat_length[0] == idat_length[best])
                    sprintf(str, "     (no critical chunk change)\n");
                else if (idat_length[0] > idat_length[best])
                    sprintf(str, "     (%4.2f%% critical chunk reduction)\n",
                      (100.0 - (100.0 * idat_length[best]) / idat_length[0]));
                else
                    sprintf(str, "     (%4.2f%% critical chunk increase)\n",
                      -(100.0 - (100.0 * idat_length[best]) / idat_length[0]));
                if (input_length == output_length)
                    sprintf(str, "     (no filesize change)\n\n");
                else if (input_length > output_length)
                    sprintf(str, "     (%4.2f%% filesize reduction)\n\n",
                      (100.0 - (100.0 * output_length) / input_length));
                else
                    sprintf(str, "     (%4.2f%% filesize increase)\n\n",
                      -(100.0 - (100.0 * output_length) / input_length));

                if (verbose > 2)
                    sprintf(str, "   Number of open files=%d\n",
                      number_of_open_files);
